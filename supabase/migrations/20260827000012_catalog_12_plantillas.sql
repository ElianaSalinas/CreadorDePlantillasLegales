...
-- PARTE 12 de 42: plantillas 67–72. Requiere la parte 0.
...
_______________________________          _______________________________
  ...
  FROM (VALUES
    ...
    ('parte_primera_razon_social', 25),
    ('parte_primera_representante_cargo', 26),
    ('parte_primera_rnc', 27),
    ('parte_primera_tipo_documento', 28),
    ('parte_primera_tipo_parte', 29),
    ...
    ('parte_segunda_razon_social', 45),
    ('parte_segunda_representante_cargo', 46),
    ('parte_segunda_rnc', 47),
    ('parte_segunda_tipo_documento', 48),
    ('parte_segunda_tipo_parte', 49),
    ('territorio_contrato', 50),
    ('titular_propiedad_intelectual', 51)
  ) AS t(tag, ord)
  JOIN variables v ON v.tag = t.tag AND v.org_id IS NULL
  ON CONFLICT DO NOTHING;
END $$;
...
_______________________________          _______________________________
  ...
  FROM (VALUES
    ...
    ('penalidad_diaria_porcentaje', 49)
  ) AS t(tag, ord)
  JOIN variables v ON v.tag = t.tag AND v.org_id IS NULL
  ON CONFLICT DO NOTHING;
END $$;
...
_______________________________          _______________________________
  ...
  FROM (VALUES
    ...
    ('parte_segunda_tipo_documento', 44),
    ('parte_segunda_tipo_parte', 45),
    ('titular_propiedad_intelectual', 46)
  ) AS t(tag, ord)
  JOIN variables v ON v.tag = t.tag AND v.org_id IS NULL
  ON CONFLICT DO NOTHING;
END $$;
